...
*   **Обход TCC:** Работает без всплывающих окон "� азрешить доступ к контактам" (благодаря `ent.xml`).
...
### Импорт контактов
Используйте флаг `-i` и перечислите номера через пробел. � мена контактов генерируются автоматически (например, `Bot User 1234`).
...
*   **Решение:** Убедитесь, что файл `ent.xml` находится в папке при сборке. Если вы перемещали бинарник, переподпишите его:
    ...
    ```
    � ногда помогает сброс прав TCC: `tccutil reset AddressBook`.
...
*   `contacts.m` — � сходный код (Objective-C).
